...
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"rgb(255, 255, 255)","height":"auto"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":"2px solid #cdd4de","padding":"5px 0","margin":"0 10px","borderRadius":"40px","display":"flex"}' v-if="isAuth('fangwuchuzu','首页总数')">
			...
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#8d9cb5","fontWeight":"bold","height":"24px"}'>{{fangwuchuzuCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#8d9cb5","height":"24px"}'>房屋出租总数</div>
			</div>
		</div>
		<div :style='{"border":"2px solid rgb(205, 212, 222)","padding":" 5px 0px","margin":"0 10px","borderRadius":"40px","display":"flex"}' v-if="isAuth('fangwuchushou','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"red","justifyContent":"center","display":"none","height":"80px"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"rgb(141, 156, 181)","fontWeight":"bold","height":"24px"}'>{{fangwuchushouCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"rgb(141, 156, 181)","height":"24px"}'>房屋出售总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="fangwuchuzuChart1" class="echarts1" v-if="isAuth('fangwuchuzu','首页统计')"></div>
		<div id="fangwuchushouChart1" class="echarts2" v-if="isAuth('fangwuchushou','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getfangwuchuzuCount();
		this.fangwuchuzuChat1();
		this.getfangwuchushouCount();
		this.fangwuchushouChat1();
	},
	methods:{
		// 词云
		...
		getfangwuchuzuCount() {
			this.$http({
				url: `fangwuchuzu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.fangwuchuzuCount = data.data
				}
			})
		},
		fangwuchuzuChat1() {
        ...
        var fangwuchuzuChart1 = echarts.init(document.getElementById("fangwuchuzuChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                fangwuchuzuChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    fangwuchuzuChart1.resize();
                };
            }
        });
      })
		...
		getfangwuchushouCount() {
			this.$http({
				url: `fangwuchushou/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.fangwuchushouCount = data.data
				}
			})
		},
		fangwuchushouChat1() {
        ...
        var fangwuchushouChart1 = echarts.init(document.getElementById("fangwuchushouChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                fangwuchushouChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    fangwuchushouChart1.resize();
                };
            }
        });
      })
