		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('chushouhetong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#8a99b2","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('chushouhetong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu8" :style='{"margin":"0 2px","fontSize":"14px","color":"#8e9db5","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('chushouhetong','打印')" type="success" @click="printJson()">
			...
			<div :style='{"width":"100%","padding":"0","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="chushouhetong" label="出售合同">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('chushouhetong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('chushouhetong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('chushouhetong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
...
import AddOrUpdate from "./add-or-update";
	...
	chushouhetongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'chushouhetong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    async printJson() {
      ...
      for (let i=0; i < this.list.length; i++) {
          data.push({
          chushoubianhao: this.list[i].chushoubianhao,
          fangwumingcheng: this.list[i].fangwumingcheng,
          fangwuleixing: this.list[i].fangwuleixing,
          fangwumianji: this.list[i].fangwumianji,
          shoujia: this.list[i].shoujia,
          fangwudizhi: this.list[i].fangwudizhi,
          huzhu: this.list[i].huzhu,
          lianxifangshi: this.list[i].lianxifangshi,
          gongsizhanghao: this.list[i].gongsizhanghao,
          gongsimingcheng: this.list[i].gongsimingcheng,
