				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="view" v-if="isAuth('emailregistercode','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
...
import AddOrUpdate from "./add-or-update";
	...
	emailregistercodestatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'emailregistercode/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
