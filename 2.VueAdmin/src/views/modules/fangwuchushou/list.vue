<template>
	<div class="main-content" :style='{"width":"100%","padding":"30px","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">房屋名称</label>
						<el-input v-model="searchForm.fangwumingcheng" placeholder="房屋名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="出售状态" prop="chushouzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">出售状态</label>
						<el-select clearable v-model="searchForm.chushouzhuangtai" placeholder="请选择出售状态" >
							<el-option v-for="(item,index) in chushouzhuangtaiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fangwuchushou','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#8a99b2","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('fangwuchushou','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('fangwuchushou','房屋出售统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='false' prop="fangwufengmian" width="200" label="房屋封面">
						<template slot-scope="scope">
							<div v-if="scope.row.fangwufengmian">
								<img v-if="scope.row.fangwufengmian.substring(0,4)=='http'" :src="scope.row.fangwufengmian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.fangwufengmian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false'  
						prop="chushouzhuangtai"
						label="出售状态">
						<template slot-scope="scope">
							{{scope.row.chushouzhuangtai}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false'  
						prop="gongsizhanghao"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fangwuchushou','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('fangwuchushou','预约看房')" @click="yuyuekanfangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('fangwuchushou','房屋购买')" @click="fangwugoumaiCrossAddOrUpdateHandler(scope.row,'cross','','','chushouzhuangtai','已出售','已出售,未出售'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"32px"}'></span>
								房屋购买
							</el-button>
							<el-button class="edit" v-if=" isAuth('fangwuchushou','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('fangwuchushou','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<fangwugoumai-cross-add-or-update v-if="fangwugoumaiCrossAddOrUpdateFlag" :parent="this" ref="fangwugoumaiCrossaddOrUpdate"></fangwugoumai-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="chushouzhuangtaiChart1" style="width:100%;height:600px;"></div>
...
import fangwugoumaiCrossAddOrUpdate from "../fangwugoumai/add-or-update";
	export default {
		data() {
			return {
				...
				fangwugoumaiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yuyuekanfangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','fangwuchushou');
			...
			fangwugoumaiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fangwugoumaiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fangwuchushou');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fangwugoumaiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fangwugoumaiCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var chushouzhuangtaiChart1 = echarts.init(document.getElementById("chushouzhuangtaiChart1"),'macarons');
        this.$http({
            url: "fangwuchushou/group/chushouzhuangtai",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].chushouzhuangtai);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].chushouzhuangtai
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                chushouzhuangtaiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chushouzhuangtaiChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.chushouzhuangtai!='' && this.searchForm.chushouzhuangtai!=undefined){
            params['chushouzhuangtai'] = this.searchForm.chushouzhuangtai
          }
	...
	fangwuchushoustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'fangwuchushou/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
