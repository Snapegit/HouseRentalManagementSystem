	...
	<div class="addEdit-block" :style='{"width":"100%","padding":"50px 0 30px 0","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="使用权" prop="shiyongquan">
					<el-input v-model="ruleForm.shiyongquan" placeholder="使用权" clearable  :readonly="ro.shiyongquan"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="使用权" prop="shiyongquan">
					<el-input v-model="ruleForm.shiyongquan" placeholder="使用权" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="租赁状态" prop="zulinzhuangtai">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="租赁状态" prop="zulinzhuangtai">
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `fangwuchuzu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fangwuchuzuCrossAddOrUpdateFlag = false;
											this.parent.search();
