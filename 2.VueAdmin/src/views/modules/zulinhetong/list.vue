			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">租赁编号</label>
						<el-input v-model="searchForm.zulinbianhao" placeholder="租赁编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('zulinhetong','打印')" type="success" @click="printJson()">
			...
			<div :style='{"width":"100%","padding":"0","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="hetongwenjian" label="合同文件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.hetongwenjian" type="text" size="small" @click="download(scope.row.hetongwenjian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('zulinhetong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(139, 154, 179)","display":"none","height":"40px"}'></span>
								修改
							</el-button>
...
import AddOrUpdate from "./add-or-update";
	...
	zulinhetongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zulinhetong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    async printJson() {
      ...
      for (let i=0; i < this.list.length; i++) {
          ...
          zhongzhiriqi: this.list[i].zhongzhiriqi,
