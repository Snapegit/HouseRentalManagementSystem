	...
	<div>
		<div class="container" :style='{"minHeight":"100vh","width":"100%","alignItems":"center","background":"url(http://codegen.caihongy.cn/20240126/6145e2c35351411a81426b78728fd386.jpg)no-repeat center center / cover","justifyContent":"center","display":"flex"}'>
			<el-form v-if="pageFlag=='register'" :style='{"padding":"100px 20px 20px 50px","boxShadow":"0 1px 6px rgba(64, 158, 255, .8)","margin":"0","borderRadius":"10px","background":"#fff","width":"600px","position":"relative","height":"auto"}' ref="rgsForm" class="rgs-form" :model="rgsForm" :rules="rules">
				<div v-if="true" :style='{"margin":"0 0 10px 0","color":"#fff","top":"0px","textAlign":"center","left":"0px","background":"rgb(51, 136, 203)","width":"100%","letterSpacing":"3px","lineHeight":"60px","fontSize":"24px","position":"absolute","fontWeight":"600"}' class="title">房屋租赁管理系统注册</div>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='dichangongsi'">
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='dichangongsi'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('tupian')?'required':''">图片：</div>
                    <file-upload
                        tip="点击上传图片"
                        action="file/upload"
                        :limit="3"
                        ...
                        @change="dichangongsitupianUploadChange"
                    ></file-upload>
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='dichangongsi'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('lianxiren')?'required':''">联系人：</div>
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='dichangongsi'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('gongsidizhi')?'required':''">公司地址：</div>
					<el-input  v-model="ruleForm.gongsidizhi"  autocomplete="off" placeholder="公司地址"  type="text"  />
				</el-form-item>
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item email" v-if="tableName=='yonghu'">
					...
					<div style="display: flex;flex: 1;">
						<input :style='{"border":" 3px solid rgb(201, 208, 220)","padding":"0 12px","margin":"0","outline":"none","color":"#606266","borderRadius":"4px 0 0 4px","background":"#fff","width":"290px","fontSize":"14px","height":"44px"}' v-model="ruleForm.email" autocomplete="off" placeholder="邮箱"/>
						<button :style='{"border":"0","cursor":"pointer","padding":"0","margin":"0","outline":"none","color":"#fff","borderRadius":"0 4px 4px 0","background":"rgb(51, 136, 203)","width":"108px","fontSize":"12px","height":"44px"}' v-if="isEndFlag" type="success" class="register-code" @click="sendemailcode()">发送验证码</button>
						<button :style='{"border":"0","cursor":"pointer","padding":"0","margin":"0","outline":"none","color":"#fff","borderRadius":"0 4px 4px 0","background":"rgb(51, 136, 203)","width":"108px","fontSize":"12px","height":"44px"}' v-if="!isEndFlag" type="success" class="register-code" disabled="disabled">{{SecondToDate}}</button>
					</div>
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item email-code" v-if="tableName=='yonghu'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('email')?'required':''">验证码：</div>
...
export default {
	...
	mounted(){
		...
		if(this.$route.query.pageFlag=='register'){
			...
			if(this.tableName=='dichangongsi'){
				this.ruleForm = {
					gongsizhanghao: '',
					gongsimingcheng: '',
					mima: '',
			...
			if ('dichangongsi' == this.tableName) {
				this.rules.gongsizhanghao = [{ required: true, message: '请输入公司账号', trigger: 'blur' }]
			}
			if ('dichangongsi' == this.tableName) {
				this.rules.gongsimingcheng = [{ required: true, message: '请输入公司名称', trigger: 'blur' }]
			}
			if ('dichangongsi' == this.tableName) {
	...
	methods: {
        ...
        dichangongsitupianUploadChange(fileUrls) {
		...
		login() {
			var url=this.tableName+"/register";
					if((!this.ruleForm.gongsizhanghao) && `dichangongsi` == this.tableName){
						this.$message.error(`公司账号不能为空`);
						return
					}
					if((!this.ruleForm.gongsimingcheng) && `dichangongsi` == this.tableName){
						this.$message.error(`公司名称不能为空`);
						return
					}
					if((!this.ruleForm.mima) && `dichangongsi` == this.tableName){
						this.$message.error(`密码不能为空`);
						return
					}
					if((this.ruleForm.mima!=this.ruleForm.mima2) && `dichangongsi` == this.tableName){
            ...
            if(this.ruleForm.tupian!=null) {
                this.ruleForm.tupian = this.ruleForm.tupian.replace(new RegExp(this.$base.url,"g"),"");
					...
					if(`dichangongsi` == this.tableName && this.ruleForm.lianxidianhua &&(!this.$validate.isMobile(this.ruleForm.lianxidianhua))){
