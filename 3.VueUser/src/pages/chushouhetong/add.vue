    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="出售编号" prop="chushoubianhao">
            <el-input v-model="ruleForm.chushoubianhao" 
                placeholder="出售编号" clearable :disabled=" false  ||ro.chushoubianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="签订日期" prop="qiandingriqi">
              <el-date-picker
				  :disabled=" false  ||ro.qiandingriqi"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="出售合同" prop="chushouhetong">
            <file-upload
            tip="点击上传出售合同"
            action="file/upload"
            :limit="1"
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          chushouhetong: [
            { required: true, message: '出售合同不能为空', trigger: 'blur' },
          ],
        },
		centerType: false,
      };
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chushouhetong/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chushouhetong/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chushouhetong/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
