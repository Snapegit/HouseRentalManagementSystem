...
<div :style='{"width":"100%","padding":"100px 18% 60px","margin":"0px auto","position":"relative","background":"url(http://codegen.caihongy.cn/20231029/b89b677f7bba41d694305c19dc3b820d.jpg) no-repeat center top,#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="出售编号" prop="chushoubianhao">
              <el-input v-model="ruleForm.chushoubianhao" placeholder="出售编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="售价" prop="shoujia">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="出售时间" prop="chushoushijian">
              <el-date-picker
				  :disabled=" false  ||ro.chushoushijian"
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.chushoushijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`fangwugoumai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			if(this.ruleForm.chushoubianhao){
				this.ruleForm.chushoubianhao = String(this.ruleForm.chushoubianhao)
			}
			//更新跨表属性
			var crossuserid;
			var crossrefid;
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('fangwugoumai/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`fangwugoumai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
